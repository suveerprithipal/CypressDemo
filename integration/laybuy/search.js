...
function ThenIExpectAShopDirectoryCountOf(expectedCount){
    cy.get('[class^=shop-directory-]').not('[class*=shop-directory-module--tag-matches]').should('have.length', expectedCount);
}
...
// Test that Stirling produces 2 entries in the shop directory
    describe('Stirling should return 2 results', () => {
        it('I search for Stirling and expect 2 results', (shop = 'Stirling', expectedCount = 2) => {
            WhenISearchForAMerchant(shop);
            ThenIExpectAShopDirectoryCountOf(expectedCount);
        })
    });
...
// Side Quest: Ensure that we can reuse the test method and that it works for other shops
    ...
    describe('SideQuest: PBTech should return 1 result', () => {
        it('I search for PbTech and expect 1 results', (shop = 'PbTech', expectedCount = 1) => {
            WhenISearchForAMerchant(shop);
            ThenIExpectAShopDirectoryCountOf(expectedCount);
        })
    });
